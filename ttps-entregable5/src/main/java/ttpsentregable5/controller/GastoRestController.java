...
import ttpsentregable5.DTO.UpdtGastoDTO;
...
//			}
	...
	@PutMapping("/{id}")
	public ResponseEntity<String> actualizarGasto(@PathVariable Long id, @RequestBody UpdtGastoDTO gastoDTO) {
		try {
			
						
			if( gastoDTO.getImagen() == null || gastoDTO.getCategoria() == null || gastoDTO.getCargaGasto()==null || gastoDTO.getRealizaGasto()==null){
			...
			Usuario nuevoCargaGasto = usuarioService.obtenerPorId(gastoDTO.getCargaGasto());
			...
			Usuario nuevoRealizaGasto = usuarioService.obtenerPorId(gastoDTO.getRealizaGasto());
			...
			// Verificar categoria
			Categoria nuevaCategoria = categoriaService.obtenerCategoriaDeGastoPorNombre(gastoDTO.getCategoria());
